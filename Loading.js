...
const Loading = ({
  ...
  // useLocation hook pulls in the query string from the URL, ex: ?entryPoint=mortgage, and saves to `search` variable
  ...
  const proceedButton = (
    <Link to={`/video${search}`} className={`btn  btn-lg ${connected ? 'btn-success' : 'btn-dark disabled'}`}>
    {/* Link to destination should append the query string retrived above so that it is accessible in the next component when clicking on the link/button */}
      {
          connected
            ? (
              <div>
  ...
  const pages = [
    ...
    <div className="card" key="page-2">
      <div className="card-body">
        ...
        <p>
          <b>
            Also, the speed of your internet connection can have a big impact on picture
